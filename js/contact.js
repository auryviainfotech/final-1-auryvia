// ===== APPOINTMENT MODAL LOGIC (copied from index.js for contact page) =====
...
const APPOINTMENT_RULES = {
  zoom: { duration: 60, startHour: 10, endHour: 17, lunchBlocked: true, allowLunch: false, gapAfter: 0 },
  inperson: { duration: 120, startHour: 10, endHour: 17, lunchBlocked: true, allowLunch: false, gapAfter: 60 },
  telecall: { duration: 60, startHour: 10, endHour: 21, lunchBlocked: false, allowLunch: true, gapAfter: 0 }
};
...
function generateAppointmentSlots(type, date) {
  ...
  while (currentMinutes + rule.duration <= endMinutes) {
    ...
    if (rule.lunchBlocked && slotStart < LUNCH_END && slotEnd > LUNCH_START) {
      currentMinutes = LUNCH_END * 60;
      continue;
    }
    slots.push({ start: formatTime(currentMinutes), end: formatTime(currentMinutes + rule.duration) });
    currentMinutes += rule.duration + rule.gapAfter;
  }
  return slots;
}
function renderAppointmentSlots() {
  ...
  if (!selectedAppointmentType || !selectedAppointmentDate) {
    timeSlotsContainer.innerHTML = '<p class="slot-hint">Select meeting type and date to see available slots</p>';
    return;
  }
  const slots = generateAppointmentSlots(selectedAppointmentType, new Date(selectedAppointmentDate));
  if (slots.length === 0) {
    timeSlotsContainer.innerHTML = '<p class="no-slots">No slots available for this date. Please select a business day (Mon-Fri).</p>';
    return;
  }
  slots.forEach(slot => {
...
appointmentForm?.addEventListener('submit', async (e) => {
  e.preventDefault();
  if (!selectedAppointmentType) { alert('Please select a meeting type'); return; }
  if (!selectedAppointmentDate) { alert('Please select a date'); return; }
  if (!selectedAppointmentSlot) { alert('Please select a time slot'); return; }
  ...
  try {
    const res = await fetch(window.API_URL + '/api/appointments', {
      method: 'POST',
